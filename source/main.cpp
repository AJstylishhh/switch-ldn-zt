...
// PC diag counterpart: pc-diag/main.cpp binds a UDP ping socket and replies
// PONG to anything starting with "PING". This is the Switch-side half of
// that same exchange, so `pcdiag.exe <switch_ip>` gets a real answer instead
// of silently getting nothing back. Port 9994 matches pc-diag's default
// target_port for instance 0.
//
// This MUST use the zts_bsd_* API, not plain socket()/bind()/recvfrom().
// Those plain POSIX calls are wrapped (-Wl,--wrap=socket etc., see Makefile)
// to talk to the real physical network interface -- that is how this app's
// own root/controller traffic already works all night, on the real wire.
// The ZeroTier VIRTUAL network (the 10.103.5.x tunnel a remote peer's
// traffic actually arrives on) is a completely separate interface, reachable
// only through zts_bsd_socket()/zts_bsd_bind()/zts_bsd_recvfrom() -- exactly
// what pc-diag/main.cpp already correctly uses on the PC side.
//
// Confirmed on hardware: with the plain-socket version, ZeroTier's own core
// received and authenticated every PING frame from a real peer just fine
// (visible in the [SWITCH-AUTH] trace as a steady stream of accepted
// VERB_FRAME packets, and a real DIRECT path got established) -- but nothing
// ever reached recvfrom() here, because this socket was never listening on
// the tunnel those frames get delivered to in the first place.
